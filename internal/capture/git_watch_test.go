...
func TestGitWatchDetectsNewCommits(t *testing.T) {
	...
	if d, _ := pollGitWatch(roots); len(d) != 0 {
...
func TestGitWatchHandlesDegenerateRoots(t *testing.T) {
	...
	if d, _ := pollGitWatch([]string{ws}); len(d) != 0 {
