...
func TestScrubEventShellCommand(t *testing.T) {
	...
	if !ok {
		t.Fatalf("event.Data lost its shape: %T", ev.Data)
	}
...
func TestScrubEventPreservesUUIDRoutingIDs(t *testing.T) {
	id, sess := event.NewUUID(), event.NewUUID()
	ev := event.Event{
		ID:        id,
		SessionID: sess,
		Kind:      "command",
	...
	ScrubEvent(&ev)
	if ev.ID != id {
		t.Errorf("routing ID was rewritten: %q -> %q", id, ev.ID)
	}
	if ev.SessionID != sess {
		t.Errorf("session ID was rewritten: %q -> %q", sess, ev.SessionID)
	}
}
